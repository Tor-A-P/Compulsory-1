...
int PlayerNumberChoice{};   /* Letting the player choose whether to play as X or O
                            against an AI*/
char PlayerCharacter{};     /* It looks cleaner if we let the user answer 'x'
                            or 'o' when we ask them who to play, so we store it
                            in a char first*/
...
int main() {
    ...
    srand(time(NULL)); /* Set the random seed to current system time, for the "AI" to
...
void Init() {
    /*Initializing the board to 1 - 9, since char is just an integer we can simply add the value of '1' to it
...
void PlayGame() {
    ...
    do {
        ...
        PlayerNumber = 1 - PlayerNumber + 2; /* Switch between player 1 and 2 every iteration of the loop
        (assuming PlayerNumber is set to either of the values already, which it is in Init)*/
...
/* Function to check for a winner of the game, it does this by checking if 3 grids in a row
(diagonally, vertically, or horizontally) are the same*/
bool CheckForWin() {
...
void PlayGameAI() {
    ...
    clearcin();
    PlayerCharacter = toupper(PlayerCharacter); // Change the letter to uppercase so we only get 2 conditions instead of 4
    ...
    while (PlayerCharacter != 'X' && PlayerCharacter != 'O') {
        ...
        std::cin >> PlayerCharacter;
        clearcin();
        PlayerCharacter = toupper(PlayerCharacter);
    }
